...
bool CPU::set_bus_ptr(Bus* bus) {
	...
	if (!m_bus) {
		msg_error("CPU bus given nullptr");
		return false;
	}

	msg_status("CPU set bus pointer");
	return true;
}
...
bool CPU::set_timing_ptr(Timing* timing) {
	...
	if (!m_timing) {
		msg_error("CPU timing given nullptr");
		return false;
	}

	msg_status("CPU set timing pointer");
	return true;
}
...
bool CPU::start_sst_mode(SST_Tester* sst_tester) {
	...
	if (tester) {
		...
		msg_error("CPU tester given nullptr");
		sst = false;
	}
...
int CPU::execute_cb_opcode(int cycles) {
